/**
 * Live integration smoke for solveTurnstile against the public 2captcha demo.
 * Not part of npm test (network + browser + non-deterministic CF).
 *
...
for (let i = 1; i <= runs; i += 1) {
  const browser = new ChromiumFishBrowser({
    ...
    allowedHosts: [],
    chromePath,
    personaSeed: `solve-test-${i}-${Date.now()}`,
  });
  const started = Date.now();
  try {
    console.log(`\n=== run ${i}/${runs} ===`);
    await browser.navigate(URL);
    // give CF a moment to paint the widget
